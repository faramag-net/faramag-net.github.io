...
function normalizeSaleItems(sale){
    ...
    const isConsignacion = sale.tipoOperacion === "CONSIGNACION" || sale.tipoOperacion === "CONSIGNACION_PARCIAL" || sale.consignacion || sale.consignacionId;
        ...
        if(c?.items?.length){
            const entregados = c.items
                ...
                .map(item => {
                    ...
                    return { productId:item.productId, nombre:p?.nombre || "Producto", entregado:cantidad, precio, importeEntregado:cantidad * precio };
                });
            ...
            const devueltos = c.items
                ...
                .map(item => {
                    ...
                    const precio = Number(item.precio ?? p?.precio ?? 0);
                    return { productId:item.productId, nombre:p?.nombre || "Producto", devuelto:cantidad, precio, importeDevuelto:-(cantidad * precio) };
                });
            ...
            if(Array.isArray(c.ventasParciales) && c.ventasParciales.length){
                ...
                // Compatibilidad con consignaciones antiguas.
            ...
            const vendidos = [...soldMap.values()];
            return { entregados, vendidos, devueltos };
        }
    }
    ...
    if(Array.isArray(sale.items) && sale.items.length){
        const vendidos = sale.items.map(item => {
            ...
            return { nombre:p?.nombre || sale.producto || "Producto", vendido:quantity, precio:price, importeVendido:quantity * price };
        });
        return { entregados:[], vendidos, devueltos:[] };
    }
